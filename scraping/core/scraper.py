...
def resolve_poi_environment(poi_name: str, location: Optional[str] = None) -> Optional[str]:
    """Resolve POI environment type via Gemini, with pois_cache caching.

    Returns one of: desert, tropical, coastal, mountain, forest, city, rural
    Used by the CLIP environment filter to reject images from wrong environments.
    """
    # Check cache first
    try:
        from shared.supabase_client import SupabaseClient
        ...
        if cache["exists"] and cache["record"]:
            ...
            if notes:
                try:
                    notes_data = json.loads(notes) if isinstance(notes, str) else notes
                    env = notes_data.get("environment")
                    if env:
                        logger.info("Environment from cache: %s → %s", poi_name, env)
                        return env
                except (json.JSONDecodeError, TypeError):
                    pass
    except Exception as exc:
        logger.warning("Environment cache lookup failed for %s: %s", poi_name, exc)
        db = None
    ...
    try:
        ...
        resp = model.generate_content(
            f'What is the primary natural environment/setting of the hotel "{poi_name}"{loc_hint}? '
            f'Reply with ONLY one word from this list: desert, tropical, coastal, mountain, forest, city, rural. '
            f'Choose the BEST match. No explanation.'
        )
        env = resp.text.strip().lower().rstrip(".")
        valid_envs = {"desert", "tropical", "coastal", "mountain", "forest", "city", "rural"}
        if env not in valid_envs:
            logger.warning("Gemini returned invalid environment '%s' for %s — ignoring", env, poi_name)
            return None

        logger.info("Environment resolved: %s → %s", poi_name, env)
            ...
            try:
                ...
                if record["exists"] and record["record"].get("notes"):
                    try:
                        ...
                        pass
                existing_notes["environment"] = env
                db.client.table("pois_cache").update({
                    "notes": json.dumps(existing_notes),
                }).eq("poi_name", poi_name).execute()
            except Exception as exc:
                logger.warning("Failed to cache environment for %s: %s", poi_name, exc)

        return env

    except Exception as exc:
        logger.warning("Gemini environment resolution failed for %s: %s", poi_name, exc)
        return None
